...
root = tk.Tk()
root.title('Contour detection demo')
...
def update_image():
    ...
    if len(contours) > 0:
        for x in contours:
            cv2.drawContours(frame, [x], 0, (0, 255, 0), 3)
    ...
    if len(contours) > 0:
        for x in contours:
            ...
            if M['m00'] > 0:
                ...
                cv2.putText(frame, str(cv2.contourArea(x)), (cx - 20, cy - 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
